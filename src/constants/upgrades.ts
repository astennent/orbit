...
export const UPGRADE_REGISTRY: Record<UpgradeId, UpgradeEntry> = {
  ...
  [ModuleId.RAMJET]: {
    ...
    desc: `Boosts speed by ${((RAMJET_SPEED_MULTIPLIER - 1) * 100).toFixed(0)}% and restores ${RAMJET_HULL_RESTORED} hull integrity.`,
  ...
  [ModuleId.RAMJET_V2]: {
    ...
    cost: 8,
    color: 'var(--glow-green)',
    desc: `Boosts speed by ${((RAMJET_V2_SPEED_MULTIPLIER - 1) * 100).toFixed(0)}% and restores ${RAMJET_V2_HULL_RESTORED} hull integrity.`,
    blurb: "Scramjet compression manifold. Faster than light, twice as loud!",
    triggerId: TriggerId.HIT_BEACON,
    image: ramjetImg
  },
